use vm_js::{
  Heap, HeapLimits, PropertyDescriptor, PropertyKey, PropertyKind, Value, Vm, VmError, VmOptions,
};
...
fn create_data_property_or_throw_throws_on_non_extensible_object() -> Result<(), VmError> {
  ...
  let err = scope
    .create_data_property_or_throw(obj, key, Value::Number(1.0))
    .unwrap_err();
...
fn delete_property_or_throw_throws_on_non_configurable_property() -> Result<(), VmError> {
  ...
  let err = scope.delete_property_or_throw(obj, key).unwrap_err();
  assert!(matches!(err, VmError::TypeError(_)));
  Ok(())
}
...
fn get_method_returns_none_for_undefined() -> Result<(), VmError> {
  ...
  let got = vm.get_method(&mut scope, Value::Object(obj), key)?;
  assert_eq!(got, None);
  Ok(())
}
...
fn get_method_throws_type_error_for_non_callable() -> Result<(), VmError> {
  ...
  let err = vm.get_method(&mut scope, Value::Object(obj), key).unwrap_err();
  assert!(matches!(err, VmError::TypeError(_)));
  Ok(())
}
...
fn get_method_on_non_object_is_unimplemented_for_now() -> Result<(), VmError> {
  ...
  let key = PropertyKey::from_string(scope.alloc_string("m")?);
  let err = vm
    .get_method(&mut scope, Value::Number(1.0), key)
    .unwrap_err();
  assert!(matches!(
    err,
