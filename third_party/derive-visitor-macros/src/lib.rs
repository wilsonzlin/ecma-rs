...
use syn::{
    ...
    DataEnum, DataStruct, DeriveInput, Error, Expr, ExprLit, Field, Fields, Ident, Lit, LitStr,
    Member, Meta, MetaList, Path, Result, Token, Variant,
};
...
fn extract_meta(attrs: Vec<Attribute>, attr_name: &str) -> Result<Option<Meta>> {
    ...
    if let Some(second) = macro_attrs.get(1) {
        return Err(Error::new_spanned(second, "duplicate attribute"));
    }

    Ok(macro_attrs.into_iter().next().map(|attr| attr.meta))
}
...
fn visitor_method_name_from_param(param: Param, path: &Path, event: &str) -> Result<Ident> {
    match param {
        Param::StringLiteral(_, lit_str) => lit_str.parse(),
...
fn impl_visitor(input: DeriveInput, mutable: bool) -> Result<TokenStream> {
    let mut params_iter = Params::from_attrs(input.attrs, "visitor")?;
    let mut params = HashMap::new();
    for param in params_iter.by_ref() {
        let param = param?;
        let (path, item_params) = match param {
            Param::Unit(path, _) => (
                path.clone(),
                VisitorItemParams {
            ...
            Param::NestedParams(path, _, mut nested) => {
            ...
            Param::StringLiteral(_, lit) => {
                return Err(Error::new_spanned(lit, "invalid attribute"));
            }
        };
        params.insert(path, item_params);
    }
