...
fn script_rejects_function_decl_named_yield_inside_generator_body() {
  assert!(parse_with_options("function* f() { function yield() {} }", ecma_script_opts()).is_err());
}

#[test]
fn script_rejects_async_function_expr_named_await() {
  assert!(parse_with_options("let x = async function await() {};", ecma_script_opts()).is_err());
}

#[test]
fn script_rejects_generator_function_expr_named_yield() {
  assert!(parse_with_options("let x = function* yield() {};", ecma_script_opts()).is_err());
}
...
fn module_rejects_named_function_expr_named_await() {
  assert!(parse_with_options("let x = function await() {};", ecma_module_opts()).is_err());
}
