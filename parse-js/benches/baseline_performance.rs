...
fn bench_type_parsing(c: &mut Criterion) {
    let samples = vec![
        ...
        ("primitive_types", include_str!("../samples/primitive_types.ts"), 27),
        ("simple_interface", include_str!("../samples/simple_interface.ts"), 53),

        // Medium files (100-1000 lines)
        ("complex_types", include_str!("../samples/complex_types.ts"), 120),
        ("medium_file", include_str!("../samples/medium_file.ts"), 318),

        // Large files (> 1000 lines)
        ("large_file", include_str!("../samples/large_file.ts"), 836),
        ("very_large_file", include_str!("../samples/very_large_file.ts"), 2508),
    ];
...
fn bench_specific_features(c: &mut Criterion) {
    ...
    let features = vec![
        ...
        ("conditional", "type T<U> = U extends string ? true : false;"),
        ("mapped", "type T<O> = { [K in keyof O]: O[K] };"),
        ("template", "type T = `Hello ${string}`;"),
        ("tuple", "type T = [string, number, boolean];"),
        ("nested_conditional", "type T<A, B, C, D> = A extends B ? C extends D ? E : F : G extends H ? I : J;"),
        ("intersection", "type T = { a: string } & { b: number } & { c: boolean };"),
        ("indexed_access", "type T<O, K extends keyof O> = O[K];"),
        ("function_type", "type T = (a: string, b: number) => boolean;"),
        ...
        ("string_literal_union", r#"type T = "foo" | "bar" | "baz" | "qux";"#),
    ];
...
fn bench_stress_test(c: &mut Criterion) {
    ...
    group.bench_function("wide_union", |b| {
        b.iter(|| parse(black_box(&wide_union)))
    });

    group.bench_function("many_types", |b| {
        b.iter(|| parse(black_box(&many_types)))
    });
