...
fn initializer_body_can_capture_function_param() {
  ...
  let y_body = program.body_of_def(y_def).expect("expected initializer body for `y`");
  ...
  let x_offset = u32::try_from(source.find("= x").expect("`= x` in source") + 2)
    .expect("offset fits in u32");
  let (_, x_ty) = result.expr_at(x_offset).expect("expected `x` expr in initializer");
