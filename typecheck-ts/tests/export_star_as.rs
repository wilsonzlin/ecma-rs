...
#[test]
fn export_type_star_as_default_is_available_via_type_only_default_import() {
  let mut host = MemoryHost::new();
  ...
  host.insert(
    root_key.clone(),
    "export type * as default from \"./dep\";\n",
  );

  let entry_key = FileKey::new("entry.ts");
  host.insert(
    entry_key.clone(),
    "import type ns from \"./root\";\n\
     export const typed: ns.Foo = { x: 1 };\n\
     export const x = typed.x;\n",
  );
...
#[test]
fn export_star_as_default_is_available_via_type_only_named_default_import() {
  let mut host = MemoryHost::new();
  ...
  host.insert(
    entry_key.clone(),
    "import type { default as NS } from \"./root\";\n\
     declare const ns: NS;\n\
     export const v = ns.value;\n\
     export const typed: NS.Foo = { x: 1 };\n\
     export const x = typed.x;\n",
...
#[test]
fn export_type_star_as_default_is_available_via_type_only_named_default_import() {
  let mut host = MemoryHost::new();
