...
impl Env {
  ...
  pub fn get_var_state(&self, name: NameId) -> Option<VarState> {
  ...
  pub fn set_var_with_assigned(&mut self, name: NameId, ty: TypeId, assigned: bool) {
    let key = FlowKey::root(name);
    self.invalidate_prefix(&key);
    self.vars.insert(key, VarState::new(ty, assigned));
  }

  pub fn set_with_assigned(&mut self, name: NameId, ty: TypeId, assigned: bool) {
