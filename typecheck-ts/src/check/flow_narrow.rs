...
pub fn truthy_falsy_types(ty: TypeId, store: &TypeStore) -> (TypeId, TypeId) {
  ...
  match store.type_kind(ty) {
    ...
    TypeKind::Number => (ty, primitives.never),
...
fn merge_fact_maps(
  mut base: HashMap<FlowKey, TypeId>,
  other: HashMap<FlowKey, TypeId>,
  store: &TypeStore,
...
/// Combine facts for a logical `&&` expression.
pub fn and_facts(left: Facts, right: Facts, store: &TypeStore) -> Facts {
  Facts {
    truthy: merge_fact_maps(left.truthy, right.truthy, store),
    falsy: merge_fact_maps(left.falsy, right.falsy, store),
    assertions: merge_fact_maps(left.assertions, right.assertions, store),
  }
}
...
pub fn split_nullish(ty: TypeId, store: &TypeStore) -> (TypeId, TypeId) {
  ...
  match store.type_kind(ty) {
    TypeKind::Union(members) => {
      ...
      for member in members {
        let (nonnull, nulls) = split_nullish(member, store);
...
/// Separate the nullish portion of a type for narrowing.
pub fn narrow_non_nullish(ty: TypeId, store: &TypeStore) -> (TypeId, TypeId) {
  split_nullish(ty, store)
}

/// Narrow by comparing a discriminant property along a member path.
pub fn narrow_by_discriminant_path(
  ...
  if let (Some(PathSegment::String(prop)), LiteralValue::String(value)) = (path.first(), lit) {
    return narrow_by_discriminant(ty, prop, value, store);
  }
  (ty, ty)
}

/// Narrow by assignability between two types.
...
/// Narrow an `instanceof` comparison using the right-hand side type.
pub fn narrow_by_instanceof_rhs(obj: TypeId, _rhs: TypeId, store: &TypeStore) -> (TypeId, TypeId) {
  narrow_by_instanceof(obj, store)
}

/// Narrow by equality or inequality with a nullish literal.
pub fn narrow_by_nullish_equality(
  ...
  let (nonnullish, nullish) = split_nullish(ty, store);
  match op {
    BinaryOp::Equality | BinaryOp::StrictEquality => (nullish, nonnullish),
    BinaryOp::Inequality | BinaryOp::StrictInequality => (nonnullish, nullish),
    _ => {
      let prim = store.primitive_ids();
      (prim.never, ty)
    }
