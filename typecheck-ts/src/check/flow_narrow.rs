...
pub fn truthy_falsy_types(ty: TypeId, store: &TypeStore) -> (TypeId, TypeId) {
  ...
  match store.type_kind(ty) {
    ...
    // Plain strings are treated as truthy; the empty string is already covered
    // by the literal arm above. Numbers and bigints retain their falsy
    // possibilities (0/NaN/0n) unless they are represented as literals.
