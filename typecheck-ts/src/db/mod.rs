...
pub use queries::{
  ...
  DeclInfo, DeclKind, GlobalBindingsDb, Initializer, LowerResultWithDiagnostics, TsSemantics,
...
impl Database {
  ...
  pub fn set_file(
    ...
    self.set_file_with_origin(file, key, kind, text, origin);
  }

  /// Set file kind and text for a given `FileId` with explicit origin.
  pub fn set_file_with_origin(
    &mut self,
    file: FileId,
    key: FileKey,
    kind: FileKind,
