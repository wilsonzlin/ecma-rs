...
pub(crate) fn sem_hir_from_lower(ast: &Node<TopLevel>, lowered: &LowerResult) -> sem_ts::HirFile {
  ...
  sem_ts::HirFile {
    ...
    ambient_modules: collect_ambient_modules(ast.stx.body.as_slice(), lowered, &resolve_name),
  }
}
  ...
  stmts
    ...
    .filter_map(|stmt| match stmt.stx.as_ref() {
      Stmt::ModuleDecl(module) => match module.stx.name {
        ModuleName::String(_) => Some(lower_ambient_module(module, lowered, resolve_name)),
        _ => None,
      },
      _ => None,
    })
...
fn lower_ambient_module(
  ...
  let name_span = TextRange::new(
    module.stx.name_loc.start_u32(),
    module.stx.name_loc.end_u32(),
  );
  let module_span = TextRange::new(module.loc.start_u32(), module.loc.end_u32());
  let def_name = format!("\"{name}\"");
  let module_def = lowered
    .defs
    .iter()
    .find(|def| {
      matches!(def.path.kind, HirDefKind::Module) && resolve_name(def.path.name) == def_name
    })
    .map(|def| def.id);

  let mut decls = Vec::new();
  if let Some(owner) = module_def {
    for def in lowered.defs.iter().filter(|def| def.parent == Some(owner)) {
      let Some(kind) = map_def_kind(def.path.kind) else {
        continue;
      };
      let exported = if def.is_exported {
        if def.is_default_export {
      ...
      decls.push(sem_ts::Decl {
        def_id: def.id,
        name: resolve_name(def.path.name),
        kind,
        is_ambient: def.is_ambient,
        is_global: def.in_global,
        exported,
  ...
  let imports: Vec<_> = lowered
    ...
    .imports
    .iter()
    .filter(|import| import.span.start >= module_span.start && import.span.end <= module_span.end)
    .filter_map(|import| map_import_from_lower(import, resolve_name))
    .collect();
  let exports: Vec<_> = lowered
    .hir
    .exports
    .iter()
    .filter(|export| export.span.start >= module_span.start && export.span.end <= module_span.end)
    .filter_map(|export| map_export_from_lower(export, resolve_name))
    .collect();
  ...
  let ambient_modules = module
    ...
    .body
    .as_ref()
    .map(|body| collect_ambient_modules(body, lowered, resolve_name))
    .unwrap_or_default();
  sem_ts::AmbientModule {
    name,
    name_span,
